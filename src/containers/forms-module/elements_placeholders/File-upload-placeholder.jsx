...
const FileUpload = (props) => {
	...
	const changeMaxSize = (value) => {
		let reg = /^[0-9]+$/;
		if ((value.match(reg) && parseInt(value.split("")[0]) !== 0) || value.length < 1) {
						...
						<div className="width-100-pc formbuilder-prev-inpts">
							...
							<div className="mb-2-nlc-noc-child formbuilder-prev-inpts">
								...
								<label htmlFor={`${elementDetails.unique_id}`} className="upload">
									<input disabled type="file" id={`${elementDetails.unique_id}`} />
									<svg>
										<use xlinkHref="/uploads/icons.svg#clip" />
									</svg>
									<span>Upload file(videos, pdf, .csv)</span>
									{elementDetails.maxLength !== undefined && elementDetails.maxLength.length > 0 && (
