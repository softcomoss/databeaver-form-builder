...
export default function (state = INITIAL_STATE, action) {
	switch (action.type) {
		...
		case DUPLICATE_ELEMENT:
			...
			elementArrayToAddDuplicate[action.payload.page - 1].elements.splice(action.payload.index + 1, 0, {
		...
		case SET_ACTIVE_SECTION:
			...
			if (pageForTargetSection.sections.length < parseInt(action.payload.split(" ")[1])) {
		...
		case UPDATE_ELEMENT_VIEW:
			...
			tempView.map((targetPage, i) => {
				if (targetPage.label === state.activePage && targetPage.elements.length > 0) {
			...
			formToAddMaxRating.map((page, i) => {
				if (page.label === state.activePage) {
					page.elements.map((element, i) => {
						if (element.unique_id === state.activeElement) {
							element.ratings = [];
							if (action.payload.number <= 0 || action.payload.number === null) {
								element.ratings = [];
							} else {
		...
		case ADD_RULE:
			...
			formDataToAddRule.map((page, i) => {
				if (page.label === state.activePage) {
					page.elements.map((element, i) => {
						if (element.unique_id === state.activeElement) {
							element.logics.push({ condition: "", values: [], action: "", target: "" });
						}
					});
				}
			});
		...
		case SET_CONDITION:
			...
			formToAddCondition.map((page, i) => {
				if (page.label === state.activePage) {
					page.elements.map((element, i) => {
						if (element.unique_id === state.activeElement) {
							element.logics[action.payload.index].condition = action.payload.value;
						}
					});
				}
			});
				...
				if (page.label === state.activePage) {
					page.sections.map((section, i) => {
						if (section.label === state.activeSection) {
							section.elements.map((element, i) => {
								if (element.unique_id === state.activeElement) {
									element.options[action.payload.index].label = action.payload.label;
								}
							});
						}
					});
					...
					page.elements.map((element, i) => {
						if (element.unique_id === state.activeElement) {
							if (action.payload.withPhoto === true) {
								element.options.push({ label: action.payload.label, url: action.payload.url });
							} else {
		...
		case GET_FORM_TO_EDIT:
			...
			return {
				...
				currentView: formData.formData.length > 0 ? formData.formData[0].elements : [],
			};
			break;
		...
		case QUICK_NUMBER_OF_ENTRIES:
			...
			numberOfEntriesAdded.totalExpectedEntries = parseInt(action.payload.number);
			return {
				...state,
