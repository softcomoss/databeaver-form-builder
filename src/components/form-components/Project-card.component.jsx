	...
	return (
		<>
			<li className="card-list-item card-deco">
				<Link
					to={permissions.includes("Projects_View_Project") ? `/view-project/${project._id}` : "#"}
					className="card">
					<p className="top-info">
						Created {moment(project.createdAt).fromNow()} by {project.createdBy.firstName}
					</p>
					<h3
				...
				<div className="border-top card-footer d-flx-alc-jsb pl-1">
					...
					<div>
						<Link
							...
							to={"/new-dispatch"}
							className={
								permissions.includes("Dispatches_Create_Dispatch") ? "btn-icon" : "btn-icon disabled"
							}>
							...
							<div
								...
								<ul className="drop-lists none my-0">
									<li className="drop-list-item">
										<a
											href="#"
											className={permissions.includes("Projects_Edit_Project") ? "" : "disabled"}
											onClick={() => {
										...
										<div id="rename-form" data-uk-modal className="uk-modal">
											<RenameProject id={highlightedProject._id} initialName={highlightedProject.name} />
										</div>
									</li>
									<li>
										<a
